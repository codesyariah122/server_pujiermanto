...
cd squid-4*
./configure '--prefix=/usr' '--bindir=/usr/bin' '--sbindir=/usr/sbin' '--libexecdir=/usr/lib/squid' '--sysconfdir=/etc/squid' '--localstatedir=/var' '--libdir=/usr/lib' '--includedir=/usr/include' '--datadir=/usr/share/squid' '--infodir=/usr/share/info' '--mandir=/usr/share/man' '--disable-dependency-tracking' '--disable-strict-error-checking' '--enable-storeio=ufs,aufs,diskd,rock' '--enable-removal-policies=lru,heap' '--enable-stacktrace' '--enable-icap-client' '--enable-wccp' '--enable-wccpv2' '--enable-cache-digests' '--disable-select' '--enable-http-violations' '--enable-linux-netfilter' '--enable-follow-x-forwarded-for' '--disable-ident-lookups' '--enable-x-accelerator-vary' '--enable-zph-qos' '--with-logdir=/var/log/squid' '--with-swapdir=/cache' '--with-openssl' '--with-large-files' '--with-pidfile=/var/run/squid.pid' '--disable-storeid-rewrite-helpers' '--enable-snmp' '--enable-ecap' '--enable-err-languages=English' '--enable-default-err-language=English' 'PKG_CONFIG_PATH=/usr/local/lib/pkgconfig' --enable-ltdl-convenience
